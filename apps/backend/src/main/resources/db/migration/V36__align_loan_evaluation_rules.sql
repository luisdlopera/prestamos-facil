-- Aligns automatic evaluation with the domain rules.
-- Rates are received as annual percentages (18.5 means 18.5%), and loans
-- created from an approved application are the active debt records in this
-- bounded context (loan lifecycle/status was removed in V17).
...
BEGIN
    ...
    IF v_annual_rate IS NULL OR v_annual_rate < 0 THEN
        p_decision := 'REJECTED';
        p_reason := 'La tasa de interés es inválida';
        RETURN;
    END IF;

    p_max_capacity := ROUND(v_base_salary * 0.35, 2);

    -- Only loans backed by an approved application count as current debt.
    -- Recalculate each installment from its stored principal/rate/term so a
    -- frontend value or an old rounded value cannot influence the decision.
    SELECT ROUND(COALESCE(SUM(
        CASE
            WHEN l.annual_interest_rate = 0 THEN l.principal_amount / l.term_in_months
            ELSE l.principal_amount
                 * ((l.annual_interest_rate / 100.0 / 12.0)
                    * POWER(1 + l.annual_interest_rate / 100.0 / 12.0, l.term_in_months))
                 / (POWER(1 + l.annual_interest_rate / 100.0 / 12.0, l.term_in_months) - 1)
        END), 0), 2)
      INTO p_current_debt
      FROM loans l
      JOIN loan_applications existing_app
        ON existing_app.id = l.loan_application_id
     WHERE l.customer_id = v_customer_id
       AND existing_app.status = 'APPROVED';

    -- Do not clamp this value: a negative capacity must remain negative.
