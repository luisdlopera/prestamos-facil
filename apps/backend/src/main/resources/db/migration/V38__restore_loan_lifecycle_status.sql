-- Loan lifecycle is required to distinguish active approved debt from
-- completely paid, cancelled, or defaulted loans.

ALTER TABLE loans
    ADD COLUMN status VARCHAR(20) NOT NULL DEFAULT 'APPROVED';

ALTER TABLE loans
    ADD CONSTRAINT chk_loans_lifecycle_status
        CHECK (status IN ('APPROVED', 'PAID', 'CANCELLED', 'DEFAULTED'));

CREATE INDEX idx_loans_customer_status ON loans(customer_id, status);

-- Recreate the procedure after the status column exists so paid/cancelled
-- loans are excluded from current monthly debt.
...
BEGIN
    ...
    SELECT ROUND(COALESCE(SUM(
        CASE
            ...
            ELSE l.principal_amount * ((l.annual_interest_rate / 100.0 / 12.0)
                 * POWER(1 + l.annual_interest_rate / 100.0 / 12.0, l.term_in_months))
                 / (POWER(1 + l.annual_interest_rate / 100.0 / 12.0, l.term_in_months) - 1)
        END), 0), 2)
      INTO p_current_debt
      FROM loans l
      JOIN loan_applications existing_app ON existing_app.id = l.loan_application_id
